class FormValidator {
	...
	constructor(formSelector = "form", form = undefined) {
		...
		this.attribs = FormValidator.attribs || {
			required: "required",
			range: "range",
			min: "min",
			max: "max",
		...
		this.types = FormValidator.types || {
			email: "email",
			url: "url",
			float: "float",
			integer: "integer",
		...
		this.typesRegex = FormValidator.typesRegex || {
		...
		this.messages = FormValidator.messages || {
			type: {
		...
		this.errorStyles = FormValidator.errorStyles || {
			input: {
